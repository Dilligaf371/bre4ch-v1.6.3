// ── Event Feed Helpers ───────────────────────────────────────────
// Pure functions extracted from event_feed_provider for testability.
// No side effects, no state — all deterministic.
...
final int missionStartMs =
    DateTime.utc(2026, 2, 28, 2, 0, 0).millisecondsSinceEpoch;

// ── Persistence constants ────────────────────────────────────────
const String eventCacheKey = 'event_cache_v2';
const int eventRetentionDays = 365;
const int maxCachedEvents = 10000;

// ── Priority sources (trusted — bypass keyword filter) ──────────
const Set<String> prioritySources = {
...
const Map<String, Map<String, String>> sourceUrls = {
  // ── News agencies ──
...
final Set<String> officialGovHandles = sourceUrls.keys
...
const List<String> conflictTerms = [
  // ── Core conflict terms ──
...
// ── Exclusion terms (reject false positives: sport, entertainment) ──
const List<String> exclusionTerms = [
...
DateTime? parseDateRobust(String input) {
...
bool isConflictRelevant(String text) {
  final lower = text.toLowerCase();
  if (exclusionTerms.any((kw) => lower.contains(kw))) return false;
  return conflictTerms.any((kw) => lower.contains(kw));
}

/// Check if a source is a priority/trusted source (bypasses keyword filter).
bool isPrioritySource(String source) {
  final lower = source.toLowerCase();
  return prioritySources.any((p) => lower.contains(p.toLowerCase()));
}

// ── Target region detection ─────────────────────────────────────

String detectTargetRegion(String text) {
  final lower = text.toLowerCase();
...
String contentHash(String title, String source, String date) {
...
AttackEvent? liveHeadlineToEvent(Map<String, dynamic> h) {
  ...
  final srcInfo = sourceUrls[src] ?? {'name': src, 'url': ''};

  int ts = DateTime.now().millisecondsSinceEpoch;
  if (pubDate.isNotEmpty) {
    final parsed = parseDateRobust(pubDate);
    if (parsed != null) ts = parsed.millisecondsSinceEpoch;
  }

  // ── DATE GATE: reject events before mission start (28 Feb 2026) ──
  if (ts < missionStartMs) return null;

  return AttackEvent(
    id: randomId('live-evt'),
    timestamp: ts,
    type: type,
    origin: src,
    target: detectTargetRegion(title),
    status: status,
    details: title,
    source: srcInfo['name'],
...
AttackEvent liveuamapToEvent(Map<String, dynamic> e) {
...
AttackEvent? socmintGovToEvent(Map<String, dynamic> m) {
  ...
  if (!officialGovHandles.contains(normalized)) return null;

  if (timestamp < missionStartMs) return null;

  final lowerContent = content.toLowerCase();
  if (!isPrioritySource(source) && !isConflictRelevant(lowerContent)) {
    return null;
  }

  final srcInfo = sourceUrls[source] ??
  ...
  return AttackEvent(
    ...
    target: detectTargetRegion(content),
    status: status,
    details: content,
    source: displayName,
