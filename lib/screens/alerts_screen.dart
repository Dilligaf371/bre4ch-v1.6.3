// =============================================================================
// BRE4CH - Alerts Screen
// Full-screen emergency alert feed with level filter + history
// BRE4CH
// =============================================================================
...
class _AlertsScreenState extends ConsumerState<AlertsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final dismissedAlerts =
        allAlerts.where((a) => a.dismissed).toList();
    ...
    if (_levelFilter.isNotEmpty) {
      filteredActive = activeAlerts.where((a) {
        return _levelFilter.contains(_levelLabel(a.level));
      }).toList();
      filteredDismissed = dismissedAlerts.where((a) {
        return _levelFilter.contains(_levelLabel(a.level));
      }).toList();
    }
        ...
        child: Column(
          children: [
              ...
              child: Row(
                children: [
                  _statPill(
                    'EXTREME',
                    activeAlerts
                        .where((a) => a.level == AlertLevel.extreme)
                        .length,
                    Palantir.danger,
                  ),
                  const SizedBox(width: 6),
                  _statPill(
                    'SEVERE',
                    activeAlerts
                        .where((a) => a.level == AlertLevel.severe)
                        .length,
                    Palantir.orange,
                  ),
                  const SizedBox(width: 6),
                  _statPill(
                    'MODERATE',
                    activeAlerts
                        .where((a) => a.level == AlertLevel.moderate)
                        .length,
                    Palantir.warning,
                  ),
                  const Spacer(),
                  // Dismiss all button (also clears badge)
                    ...
                    GestureDetector(
                      onTap: () {
                        ref
                            ...
                            .dismissAll();
                        const MethodChannel('com.qyber.breach/badge')
                            .invokeMethod('clearBadge')
                            .catchError((_) => null);
                      },
                      child: Container(
            ...
            Padding(
              ...
              child: Row(
                children: [
                  _buildToggle('ACTIVE', !_showDismissed,
                      filteredActive.length),
                  const SizedBox(width: 8),
                  _buildToggle('HISTORY', _showDismissed,
  ...
  Widget _buildToggle(String label, bool active, int count) {
    return Expanded(
      child: GestureDetector(
        onTap: () =>
            setState(() => _showDismissed = label == 'HISTORY'),
        child: Container(
      ...
      child: GestureDetector(
        ...
        child: PalantirCard(
          ...
          child: Column(
            ...
            children: [
              // Row 1: level badge, authority, region, actions
              Row(
                children: [
                  Icon(_levelIcon(alert.level),
                      size: 16, color: isDismissed ? Palantir.textMuted : color),
                  const SizedBox(width: 6),
                  Container(
                    padding:
                  ...
                  if (!isDismissed) ...[
                    ...
                    GestureDetector(
                      ...
                      child: const Icon(Icons.close,
                          size: 16, color: Palantir.textMuted),
                    ),
                  ],
                ],
              ),
              ...
              const SizedBox(height: 8),
              // Bottom row: source, time, actions
              Row(
                children: [
                  Expanded(
                    child: Text(
